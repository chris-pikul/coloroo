 ...
 * value.
 */
export type ValueMapCallback = (value:number) => number;
 ...
 */
export type ChannelMapCallback<T extends IColor = IColor> = (chan:number, index:number, clr:T) => number;
 ...
 */
export type AlphaMapCallback<T extends IColor = IColor> = (alpha:number, clr:T) => number;

export const NoOpMapCallback:ValueMapCallback = (val:number):number => val;
